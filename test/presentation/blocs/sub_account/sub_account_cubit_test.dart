...
void main() {
  group('SubAccountCubit', () {
    ...
    group('create', () {
      ...
      test('persists account to Firestore', () async {
        ...
        final QuerySnapshot<Map<String, dynamic>> snap =
            await fakeFirestore.collection('sub_account').get();
    ...
    group('loadWithTotals — per-account aggregation', () {
      test('income and expenses aggregated correctly per sub-account',
          () async {
        await fakeFirestore.collection('sub_account').add(tPersonal.toJson());
        await fakeFirestore.collection('sub_account').add(tCompany.toJson());

        // Personal: 1000 income, 300 expense → balance 700
        await fakeFirestore.collection('transaction').add(
          TransactionEntity(
            uuid: 'tx-1',
            amount: 1000.0,
            categoryUUid: 'cat-1',
        ...
        final Map<String, SubAccountTotals> totals = state.whenOrNull(
