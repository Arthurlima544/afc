...
void main() {
  ...
  setUp(() async {
    ...
    queue = SyncQueue(firestore: fakeFirestore);
  ...
  group('enqueue', () {
    ...
    test('enqueued ops preserve insertion order', () {
      queue.enqueue(_op(uuid: 'op-a'));
      queue.enqueue(_op(uuid: 'op-b'));
      queue.enqueue(_op(uuid: 'op-c'));
  ...
  group('flush', () {
    test('creates document in Firestore for create op', () async {
      ...
      final snap = await fakeFirestore.collection('goal').get();
    ...
    test('clears queue after successful flush', () async {
      queue.enqueue(_op(collection: 'goal', payload: <String, dynamic>{'uuid': 'g-1'}));
    ...
    test('updates existing document for update op', () async {
      ...
      await fakeFirestore
          .collection('goal')
          .add(<String, dynamic>{'uuid': 'g-1', 'name': 'Old'});
    ...
    test('deletes document for delete op', () async {
      await fakeFirestore
          .collection('goal')
          .add(<String, dynamic>{'uuid': 'g-1', 'name': 'Delete me'});
  ...
  group('idempotency guard', () {
    test('create does not duplicate if doc already exists in Firestore',
        () async {
      // Pre-create the document (simulates Firestore having already synced it)
      await fakeFirestore
          .collection('transaction')
          .add(<String, dynamic>{'uuid': 'tx-1', 'title': 'Existing'});
  ...
  group('clear', () {
    test('removes all ops without replaying', () async {
      queue.enqueue(_op(collection: 'goal', payload: <String, dynamic>{'uuid': 'g-1'}));
      queue.enqueue(_op(uuid: 'op-2', collection: 'goal', payload: <String, dynamic>{'uuid': 'g-2'}));
